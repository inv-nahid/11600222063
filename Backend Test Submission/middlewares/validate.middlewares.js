...
const validateUrl = (req, res, next) => {
    ...
    if (!targetUrl) {
        Log("backend", "error", "middleware", "Target URL field is missing");
    ...
    if (!urlValidationPattern.test(targetUrl)) {
        Log("backend", "error", "middleware", `Invalid URL format: ${targetUrl}`);
    ...
    if (expirationTime) {
        ...
        if (isNaN(expirationDate.getTime()) || expirationDate <= new Date()) {
            Log("backend", "error", "middleware", `Invalid expiration date: ${expirationTime}`);
            throw new ApiError(400, "Expiration date must be set to a future time");
        }
    }

    Log("backend", "info", "middleware", `URL validation passed for: ${targetUrl}`);
    next();
};
